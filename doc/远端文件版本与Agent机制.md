...
用户手动选择的 Skill 每轮执行原生调用。强制 Skill 只在切换到某个 Agent、服务器、网页分支或上下文代次后的下一次提问检查一次；普通工作区切换沿用同一 Binding 的检查状态。已有相同内容版本时不重复安装，缺少时补齐并调用。网页 Agent 按需选择的 Skill 在读取并提交后进入相同的固定版本与原生调用链路。
