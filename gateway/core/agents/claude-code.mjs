...
// Claude Code's terminal `result.usage` is the aggregate billable usage of the
...
// terminal `result.usage` is cumulative billing data and is never used as the
// current context-window occupancy.
export const CLAUDE_CONTEXT_USAGE_SOURCE = "claude-code-current-request";
...
function sessionPermissionUpdates(value) {
  return array(value).flatMap((raw) => {
    ...
    // Claude's permission suggestions may target localSettings or
    // projectSettings, which makes an "always allow" response create
    // .claude/settings.local.json in the user's repository. EasyWork's UI
    // explicitly offers "allow for this session", and Claude's native
...
const CLAUDE_SERVER_TOOL_RESULT_TYPES = new Set([
  "advisor_tool_result",
...
const CLAUDE_TOOL_USE_TYPES = new Set(["tool_use", "server_tool_use", "mcp_tool_use"]);
...
function claudeErrorMessage(frame) {
  const details = [frame.error, frame.errors, frame.message, frame.stderr, frame.result]
    .map((value) => errorDetail(value))
    .filter(Boolean)
    .filter((value, index, all) => all.indexOf(value) === index);
  return (details.join(" · ") || `Claude Code ${text(frame.subtype || "operation")} failed`).slice(0, 16_384);
}
...
function claudeContextTokens(usage) {
  return [usage.input, usage.cachedInput, usage.cacheWrite, usage.output, usage.reasoning]
    .reduce((total, value) => total + (Number.isFinite(Number(value)) ? Number(value) : 0), 0);
}

function claudeMessageContextUsage(usage, limit) {
  const contextUsage = normalizeContextUsage({
    used: Number.isFinite(Number(usage?.used))
      ? Number(usage.used)
      : claudeContextTokens(usage) || usage.total,
    limit: limit ?? usage?.limit,
  });
  return Number.isFinite(contextUsage.used)
    ? { ...contextUsage, source: CLAUDE_CONTEXT_USAGE_SOURCE }
    : null;
}

function updateClaudeRequestUsage(context, rawUsage, source, phase = "updated") {
  const usage = normalizeUsage(rawUsage);
  if (!Object.keys(usage).length) return;
  const previous = object(context.state.items.__claudeRequestUsage).usage || {};
  const merged = { ...previous, ...usage };
  recordItem(context.state, "__claudeRequestUsage", { usage: merged });
  const contextUsage = claudeMessageContextUsage(merged, context.state.contextUsage?.limit);
...
function nativeTaskCreateResult(context, frame, toolId, source) {
  ...
  // Claude's native task store can reuse a numeric id after a resumed CLI
...
function emitUnknownContentBlock(context, block, source) {
  emitUnmappedAgentEvent(context, "Claude Code", "内容块", block.type, source);
}
...
function completedContentItemId(state, frame, block, index) {
  ...
  const streamedIds = array(state.items[`claude-stream:${messageId}`]?.blockIds);
...
function emitAssistantContent(context, frame, content, completed = false) {
  for (let index = 0; index < content.length; index += 1) {
    ...
    if (block.type === "text" && block.text) {
    ...
    } else if (CLAUDE_TOOL_USE_TYPES.has(block.type)) {
      ...
      // Claude's Todo/Task tools feed the dedicated native plan surface. They
    ...
    } else if (CLAUDE_SERVER_TOOL_RESULT_TYPES.has(block.type)) {
...
function emitStreamEvent(context, frame) {
  ...
  if (eventType === "message_start") {
    ...
    recordItem(context.state, `claude-stream:${context.state.streamMessageId}`, { blockIds: [] });
    updateClaudeRequestUsage(context, event.message?.usage, sourceOf(`stream_event/${eventType}`, frame, { itemId: context.state.streamMessageId }));
    return;
  }
  // Claude's include-partial-messages stream may assign a fresh wrapper UUID
  ...
  if (eventType === "content_block_start") {
    ...
    const messageKey = `claude-stream:${messageId}`;
    ...
    recordItem(context.state, itemId, {
      ...
      // index across consecutive Claude tool-loop messages. Clear per-block
      // accumulators so JSON/text from the previous native block cannot bleed
      // into the next one.
      ...(block.type === "thinking" ? { reasoning: "" } : {}),
      ...(block.type === "text" ? { text: "" } : {}),
      ...(CLAUDE_TOOL_USE_TYPES.has(block.type) ? { partialJson: "" } : {}),
    });
    ...
    if (delta.type === "text_delta") {
      ...
      // The signature is opaque model-verification data. Claude Code owns it;
      // EasyWork neither renders nor persists it.
    } else {
      emitUnmappedAgentEvent(context, "Claude Code", "内容增量", delta.type, source);
    }
  } else if (eventType === "content_block_stop") {
    const known = object(context.state.items[itemId]);
    if (!["text", "thinking", ...CLAUDE_TOOL_USE_TYPES].includes(known.partType)) return;
    const kind = known.partType === "thinking" ? "reasoning" : CLAUDE_TOOL_USE_TYPES.has(known.partType) ? "tool_call" : "message";
    ...
    // content_block_stop only says Claude finished serializing the tool input;
    ...
    updateClaudeRequestUsage(context, event.usage, source);
  } else if (eventType === "message_stop") {
    context.emit("status", "completed", { status: "message_complete" }, source);
    context.state.streamMessageId = null;
  } else {
    emitUnmappedAgentEvent(context, "Claude Code", "流事件", eventType, source);
  }
}
...
function emitResult(context, frame) {
  ...
  if (Object.keys(usage).length) {
    ...
    const latestRequestUsage = context.state.contextUsage?.source === CLAUDE_CONTEXT_USAGE_SOURCE
      ? claudeMessageContextUsage(context.state.contextUsage, reportedLimit ?? context.state.contextUsage.limit)
      : context.state.contextUsage?.source === "claude-code-context-report"
  ...
  if (!terminalResult) {
    // Claude emits one result per native turn. When another user send is
  ...
  if (failed) {
    context.emit("error", "failed", { message: claudeErrorMessage(frame) }, sourceOf(frame.type, frame));
  } else {
...
function emitClaudeSystem(context, frame) {
  ...
  if (subtype === "init") {
    ...
    delete context.state.items.__claudeRequestUsage;
  ...
  if (["commands_changed", "thinking_tokens", "post_turn_summary", "task_summary"].includes(subtype)) return;
  emitUnmappedAgentEvent(context, "Claude Code", "system 事件", subtype, source);
}

function emitClaudeSpecial(context, frame) {
  ...
  if (type === "tool_progress") {
    ...
    const heartbeatParentId = frame.heartbeat === true
      ...
      : reportedToolId;
    // Claude Code 2.1.245 emits long-running tool heartbeats with synthetic
...
function reduceClaudeCode(previousState, frame, producer) {
  ...
  if (type === "easywork_native_boundary") {
    // Transport resolves Claude's authoritative JSONL leaf because the
    ...
    emitAgentCompatibilityIssue(context, "Claude Code", {
      ...object(frame.issue),
    ...
    emitClaudeSystem(context, frame);
    ...
    if (frame.context_usage) {
      ...
      if (Object.keys(contextUsage).length) {
        context.state.contextUsage = { ...contextUsage, source: "claude-code-context-report" };
        context.emit("usage", "completed", { context: context.state.contextUsage }, sourceOf(type, frame));
      }
    }
    if (!frame.context_usage) updateClaudeRequestUsage(context, frame.message?.usage, sourceOf(type, frame));
    ...
    if (request.subtype === "can_use_tool" && tool === "AskUserQuestion") {
      ...
      context.emit("error", "failed", {
        message: `Claude Code${agentVersion ? ` ${agentVersion}` : " 当前版本"} 发出了无法安全代答的控制请求：${eventType}`,
    ...
    for (const requestFrame of pending) {
      ...
      const nested = reduceClaudeCode(context.state, requestFrame, producer);
    ...
    if (!emitClaudeSpecial(context, frame)) emitUnmappedAgentEvent(context, "Claude Code", "事件", type, sourceOf(type, frame));
  }
...
function buildClaudeCodeOperation(operation, input) {
      ...
      frames: [
        {
          ...
          request_id: `easywork-steer-${commandId}`,
          request: { subtype: "interrupt" },
        },
        userFrame(requireString(input, "prompt", operation)),
      ],
    };
  }
  ...
  if (operation === "respondApproval") {
    ...
    invariant(["approve", "approve_session", "reject"].includes(decision), "AGENT_APPROVAL_DECISION_INVALID", "Claude Code 审批结果无效", { status: 400 });
  ...
  if (operation === "respondInput") {
    ...
    invariant(answers && typeof answers === "object" && !Array.isArray(answers), "AGENT_INPUT_ANSWERS_INVALID", "Claude Code 用户回答无效", { status: 400 });
    if (pendingInput.type === "claude_elicitation") {
    ...
    if (pendingInput.type === "claude_dialog") {
      const raw = answers.response;
      const values = Array.isArray(raw) ? raw.map(String) : [String(raw ?? "")];
      invariant(values.some((value) => value.trim()), "AGENT_INPUT_ANSWERS_REQUIRED", "Claude Code 用户回答不能为空", { status: 400 });
      return {
        transport: "process-jsonl-stdin",
        frames: [{
          type: "control_response",
    ...
    invariant(Object.keys(cleanAnswers).length > 0, "AGENT_INPUT_ANSWERS_REQUIRED", "Claude Code 用户回答不能为空", { status: 400 });
    return {
      transport: "process-jsonl-stdin",
      frames: [{
        type: "control_response",
