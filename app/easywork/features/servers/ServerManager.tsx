...
function Status({ value }: { value: ServerStatus }) {
  const labels = { connected: "已连接", connecting: "连接中", disconnected: "未连接", failed: "连接失败" };
  return <span className={`${styles.status} ${styles[value]}`}><i />{labels[value]}</span>;
}
...
function persistedConnectionError(error: ServerRecord["connection"]["lastError"]) {
  if (!error) return null;
  if (error.code === "SSH_AUTH_FAILED" || /All configured authentication methods failed/i.test(error.message)) {
    return "SSH 认证失败，请检查用户名、登录凭据和动态验证码";
  }
...
export function ServerEditor({ server, onClose, onSaved, embedded = false }: { server?: ServerRecord; onClose: () => void; onSaved: (serverId?: string) => Promise<void>; embedded?: boolean }) {
  ...
  const revealPassword = async () => {
    ...
    try {
      ...
      if (result.data.method === "password" && result.data.password) {
        ...
        revealTimer.current = window.setTimeout(() => setShowPassword(false), Math.max(0, Date.parse(result.data.expiresAt) - Date.now()));
      }
  ...
  const save = async () => {
    ...
    try {
      ...
      if (server) await runtime.api.patch(`/api/servers/${server.profile.id}`, { ...profile, ...(credentialPresent ? { credential } : {}) }, { expectedRevision: server.profile.revision });
      else {
  ...
  const complete = draft.name.trim() && draft.host.trim() && draft.username.trim() && Number(draft.port) > 0 && (editing || credentialPresent);
  const form = <form className={`${styles.editor} ${embedded ? styles.embeddedEditor : ""}`} onSubmit={(event) => { event.preventDefault(); void save(); }}>
      ...
      <section className={styles.authBox}>
        ...
        {draft.authMethod === "password" ? <label><span>登录密码</span><div className={styles.passwordWrap}><input type={showPassword ? "text" : "password"} value={draft.password} placeholder={editing ? "••••••••••••" : "输入密码"} onChange={(event) => setDraft({ ...draft, password: event.target.value })} /><button type="button" aria-label={showPassword ? "隐藏密码" : "显示密码"} disabled={revealingCredential} onClick={() => void revealPassword()}>{revealingCredential ? <LoaderCircle className={styles.spin} size={16} /> : showPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></div></label> : <div className={styles.keyFields}><input ref={keyInput} className={styles.hiddenInput} type="file" onChange={(event) => void keyFile(event)} /><button type="button" className={styles.keyPicker} onClick={() => keyInput.current?.click()}><FileKey2 size={19} /><span>{draft.privateKey ? draft.fileName : editing ? "当前私钥已保存；点击可替换" : "选择私钥文件"}</span><ChevronRight size={16} /></button><label><span>私钥密码</span><input type="password" value={draft.passphrase} placeholder="可选" onChange={(event) => setDraft({ ...draft, passphrase: event.target.value })} /></label></div>}
      </section>
      <footer className={styles.editorActions}><Button type="button" onClick={onClose}>取消</Button><Button type="submit" variant="primary" disabled={!complete || busy} icon={busy ? <LoaderCircle className={styles.spin} size={16} /> : <Save size={16} />}>{busy ? "保存中" : "保存"}</Button></footer>
    </form>;
...
function Detail({ server, onEdit, onConnect, onDisconnect }: { server: ServerRecord; onEdit: () => void; onConnect: () => void; onDisconnect: () => Promise<void> }) {
  ...
  return <div className={styles.detail}>
    ...
    {server.connection.lastError ? <div className={styles.errorBanner}><CircleAlert size={17} /><span>{persistedConnectionError(server.connection.lastError)}</span></div> : null}
    ...
    <section className={styles.bindingSection}><div className={styles.subheading}><UsersRound size={17} /><h3>连接对话</h3></div>{server.conversationIds.length ? <div className={styles.bindingList}>{server.conversationIds.map((id) => <div key={id}><span>{id}</span><i>{connected ? "SSH 可用" : "等待连接"}</i></div>)}</div> : <div className={styles.noBindings}>还没有对话使用这台服务器</div>}</section>
...
export default function ServerManager() {
  ...
  const load = useCallback(async (preferred?: string) => {
    try {
      ...
      setSelectedId((current) => preferred || (current && result.data.some((item) => item.profile.id === current) ? current : result.data[0]?.profile.id ?? null));
  ...
  if (loading) return <LoadingState label="正在读取远程服务器" />;
  return <PageFrame icon={<Server size={19} />} title="远程服务器" count={servers.length}>
    <div className={styles.manager}>
      <aside className={styles.serverList}><div className={styles.serverItems}>{servers.map((server) => <button key={server.profile.id} className={server.profile.id === selectedId ? styles.selectedServer : ""} onClick={() => { setSelectedId(server.profile.id); setEditing(null); }}><span className={styles.listIcon}><Server size={18} /></span><span><strong>{server.profile.name}</strong><small>{server.profile.host}</small></span><Status value={server.connection.status} /></button>)}</div><button className={styles.addServer} onClick={() => setEditing("new")}><Plus size={16} />添加服务器</button></aside>
      <section className={styles.detailPanel}>{editing ? <ServerEditor embedded server={editing === "selected" ? selected ?? undefined : undefined} onClose={() => setEditing(null)} onSaved={async (serverId) => { await load(serverId); await refreshBootstrap(); }} /> : selected ? <Detail server={selected} onEdit={() => setEditing("selected")} onConnect={() => setConnecting(true)} onDisconnect={disconnect} /> : <div className={styles.emptyDetail}><Server size={25} /><strong>选择或添加一台服务器</strong></div>}</section>
    </div>
